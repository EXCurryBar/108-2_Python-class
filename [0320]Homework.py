...
def distance_calc(coord1=[], coord2=[]):
    ...
    def parse_data(coordinate): return [
        radians(float(num)) for num in coordinate]

    if len(coord1) == 0 or len(coord2) == 0:
        coordinate1 = input(
            "Enter point 1(latitude and longitude) in degrees:").split(",")
        coordinate2 = input(
            "Enter point 2(latitude and longitude) in degrees:").split(",")
    else:
        coordinate1 = coord1
        coordinate2 = coord2
